...
folder_name = "libero_goal_OOD_remove_object"
task_bddl_files = glob.glob(f"./libero/libero/bddl_files/{folder_name}/*.bddl")
...
for i in range(len(task_bddl_files)):
    env_args = {"bddl_file_name": task_bddl_files[i], "camera_heights": 256, "camera_widths": 256, "camera_pos_offset":[0, 0, 0], "camera_ori_offset":[0, 0, 0]}
    env = OffScreenRenderEnv(**env_args)
    env.seed(0)
    env.reset()
    ...
    cv2.imwrite(f"./scene_image/{folder_name}/{file_name}.png", cv2.cvtColor(agent_view, cv2.COLOR_RGB2BGR))